...
import { switchMap, map, mergeMap, tap, concatMap, take } from 'rxjs/operators';
...
(() =>{
    ...
    getRequest(`${SW_API}/people/1/`).pipe(
       ...
       // tap(val =>console.log('en el tap',val)),
        mergeMap(val =>getRequest(val.starships[0]))
    // NO TOCAR el subscribe ni modificarlo ==
    ).subscribe( console.log )           // ==
    // =======================================


     //Solucion parte 1 y 2
    // Realizar el llamado al URL para obtener a Luke Skywalker
    getRequest(`${SW_API}/people/1/`).pipe(
        // Realizar los operadores respectivos aquí
        mergeMap(val =>
           zip(of(val),getRequest(val.starships[0])) 
        ),map(([personaje,nave]) => {return {personaje,nave} })
    // NO TOCAR el subscribe ni modificarlo ==
    ).subscribe( console.log )           // ==
